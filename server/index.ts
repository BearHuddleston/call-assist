...
function emit(record: CallRecord, type: CallEvent["type"], data: Record<string, unknown>) {
  record.events.push({
...
function scheduleCleanup(record: CallRecord) {
...
app.get("/health", async () => ({
  ...
  publicUrlConfigured: Boolean(process.env.TELEPHONY_PUBLIC_BASE_URL),
}));
...
app.post(
  ...
  async (request, reply) => {
    ...
    if (command.type === "call.end") {
      ...
      if (record.transport && record.instructions) {
...
app.post("/twilio/status/:callId", async (request, reply) => {
  ...
  if (record.status === "completed" || record.status === "failed" || record.status === "busy" || record.status === "no-answer") {
    record.session?.close();
    scheduleCleanup(record);
  }
...
app.get("/twilio/media/:callId", { websocket: true }, (socket, request) => {
  ...
  session.on("tool_approval_requested", (_context, _agent, approvalRequest) => {
    ...
    const args = approvalRequest.approvalItem.arguments
      ? JSON.parse(approvalRequest.approvalItem.arguments) as Record<string, unknown>
      : {};
